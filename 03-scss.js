...
gulp.task("watch-custom-scss", gulp.series('select-view', (cb) => {
    ...
    gulp.watch([config.viewRootDir() + "/**/*.scss"], {interval: 1000, usePolling: true}, gulp.series('custom-scss'));
    cb();
}));
...
//    let customScss = gulp.src(config.customScssMainPath(),{allowEmpty:true})
    let customScss = gulp.src(config.viewRootDir() + "/**/*.scss",{allowEmpty:true})
